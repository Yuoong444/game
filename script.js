...
// v63: 환생 없이도 반복 플레이가 살아나도록 만든 핵심 상태
let clickCombo = 0;
let lastClickTime = 0;
let bestCrit = 0;
let totalClicks = 0;
...
const wands = [
  { level: 0, grade: "E", name: "낡은 마법봉", stars: 1, icon: "🪵", element: "earth" },
  { level: 0, grade: "E", name: "부서진 마법봉", stars: 1, icon: "🪵", element: "normal" },
  { level: 0, grade: "E", name: "수습 마법봉", stars: 1, icon: "🪄", element: "light" },
  { level: 0, grade: "E", name: "나무 마법봉", stars: 1, icon: "🌿", element: "earth" },
  { level: 0, grade: "E", name: "빛바랜 마법봉", stars: 1, icon: "✨", element: "normal" },
  { level: 5, grade: "D", name: "견습 마법봉", stars: 2, icon: "🪄", element: "normal" },
  { level: 5, grade: "D", name: "푸른 마법봉", stars: 2, icon: "💧", element: "water" },
  { level: 5, grade: "D", name: "붉은 마법봉", stars: 2, icon: "🔥", element: "fire" },
  { level: 5, grade: "D", name: "작은 날개 마법봉", stars: 2, icon: "🪽", element: "wind" },
  { level: 5, grade: "D", name: "수정 마법봉", stars: 2, icon: "💎", element: "light" },
  { level: 10, grade: "C", name: "별빛 마법봉", stars: 3, icon: "🌟", element: "light" },
  { level: 10, grade: "C", name: "은빛 마법봉", stars: 3, icon: "🌙", element: "dark" },
  { level: 10, grade: "C", name: "바람 마법봉", stars: 3, icon: "🌪️", element: "wind" },
  { level: 10, grade: "C", name: "대지 마법봉", stars: 3, icon: "🌿", element: "earth" },
  { level: 18, grade: "B", name: "화염 마법봉", stars: 3, icon: "🔥", element: "fire" },
  { level: 18, grade: "B", name: "얼음 마법봉", stars: 3, icon: "❄️", element: "water" },
  { level: 18, grade: "B", name: "번개 마법봉", stars: 3, icon: "⚡", element: "light" },
  { level: 18, grade: "B", name: "어둠 마법봉", stars: 3, icon: "🌑", element: "dark" },
  { level: 28, grade: "A", name: "황금 마법봉", stars: 4, icon: "👑", element: "light" },
  { level: 28, grade: "A", name: "고대 마법봉", stars: 4, icon: "📜", element: "earth" },
  { level: 28, grade: "A", name: "성스러운 마법봉", stars: 4, icon: "✨", element: "light" },
  { level: 28, grade: "A", name: "폭풍 마법봉", stars: 4, icon: "🌩️", element: "wind" },
  { level: 40, grade: "S", name: "천사의 마법봉", stars: 5, icon: "🪽", element: "light" },
  { level: 40, grade: "S", name: "용의 마법봉", stars: 5, icon: "🐉", element: "fire" },
  { level: 40, grade: "S", name: "시간의 마법봉", stars: 5, icon: "⏳", element: "dark" },
  { level: 55, grade: "SS", name: "차원의 마법봉", stars: 5, icon: "🌀", element: "wind" },
  { level: 55, grade: "SS", name: "심연의 마법봉", stars: 5, icon: "🌌", element: "dark" },
  { level: 55, grade: "SS", name: "우주의 마법봉", stars: 5, icon: "🪐", element: "light" },
  { level: 75, grade: "SSS", name: "창세의 마법봉", stars: 5, icon: "🌅", element: "light" },
  { level: 75, grade: "SSS", name: "영원의 마법봉", stars: 5, icon: "♾️", element: "dark" }
];

// v69 밸런스 정리: 초중반은 답답하지 않게, SSS 후반 폭발은 완화
const gradeClickMultiplier = { E: 1.0, D: 2.0, C: 4.0, B: 7.0, A: 12.0, S: 20.0, SS: 35.0, SSS: 60.0 };
const gradeGoldBaseBonus = { E: 0, D: 80, C: 220, B: 520, A: 1200, S: 2600, SS: 6200, SSS: 15000 };
const gradeGoldLevelBonus = { E: 16, D: 28, C: 45, B: 72, A: 115, S: 190, SS: 330, SSS: 620 };
const gradeStartEnhanceCost = { E: 80, D: 150, C: 300, B: 700, A: 1600, S: 4200, SS: 11000, SSS: 28000 };
...
// v63 옵션 데이터 구조: 같은 등급이라도 플레이 스타일이 갈리도록 옵션 풀 확장
const optionTypes = [
  { key: "click", name: "클릭 수익 증가", min: 4, max: 18, weight: 22, desc: "직접 터치 수익" },
  { key: "auto", name: "자동 수익 증가", min: 3, max: 16, weight: 18, desc: "초당 골드" },
  { key: "rate", name: "강화 성공률 보너스", min: 1, max: 7, weight: 12, desc: "강화 안정성" },
  { key: "gold", name: "골드 획득량 증가", min: 5, max: 22, weight: 18, desc: "전체 골드" },
  { key: "gem", name: "보석 획득량 증가", min: 1, max: 6, weight: 10, desc: "보석 파밍" },
  { key: "crit", name: "크리티컬 확률", min: 1, max: 3, weight: 8, desc: "가끔 터지는 보너스" },
  { key: "critDmg", name: "크리티컬 피해", min: 5, max: 18, weight: 6, desc: "크리 배율" }
];
...
function cacheDom() {
  [
    ...
    "autoGemPity", "clickBonusBadge", "failPenalty", "stars", "gradeBadge", "inventoryList", "inventoryCount", "dpsIncome", "mainDpsIncome", "critChance", "mainCritChance", "comboBonus", "powerScore"
  ].forEach(id => ui[id] = $(id));
}
...
function normalizeInventoryItem(item) {
  ...
  item.savedOptions = item.savedOptions.filter(o => o && optionTypes.some(t => t.key === o.key)).slice(0, 5);
...
function syncCurrentFromEquipped() {
  ...
  options = Array.isArray(item.savedOptions) ? JSON.parse(JSON.stringify(item.savedOptions)).slice(0, 5) : [];
}
...
function syncEquippedFromCurrent() {
  ...
  item.savedOptions = Array.isArray(options) ? JSON.parse(JSON.stringify(options)).slice(0, 5) : [];
}
...
function getCritChance() {
  // 0.69.1 안정화: 기존에는 등급/옵션 보너스가 과하게 누적되어 크리티컬이 너무 자주 발생했습니다.
  const gradeRank = Math.max(0, gradeOrder.indexOf(currentWand().grade));
  const gradeBonus = gradeRank * 0.45;
  const element = normalizeAssetElement(getItemElement(getEquippedItem()));
  const elementBonus = element === "fire" ? 0.8 : element === "dark" ? 1.0 : 0;
  const optionBonus = Math.min(4, getOptionBonus("crit") * 0.55);
  return Math.round(Math.min(12, 3.5 + gradeBonus + optionBonus + elementBonus) * 10) / 10;
}

function getCritMultiplier() {
  // 0.69.1 안정화: 10배급 폭발을 막고, 기본 2배~최대 2.8배 선으로 제한합니다.
  const gradeRank = Math.max(0, gradeOrder.indexOf(currentWand().grade));
  const gradeBonus = gradeRank * 0.035;
  const optionBonus = Math.min(0.35, getOptionBonus("critDmg") / 220);
  return Math.round(Math.min(2.8, 2.0 + gradeBonus + optionBonus) * 100) / 100;
}

function getComboMultiplier() {
  return 1 + Math.min(0.60, clickCombo * 0.025);
}

function getPowerScore() {
  return Math.floor(clickIncome * 1.15 + autoIncome * 8 + getCritChance() * 95 + optionPower(getEquippedItem()) * 9 + level * 55);
}

function getSuccessRate() {
  let base;
  if (level <= 10) base = 90;
  else if (level <= 20) base = 70;
  else if (level <= 30) base = 50;
  else base = 35;
  const raw = Math.min(96, Math.max(18, base + getOptionBonus("rate") + getWeaponTrait().success + getElementSuccessBonus()));
...
function calculateIncome() {
  ...
  // getGradeClickMultiplier() 내부에 이미 getElementBonusMultiplier("click")가 포함되어 있으므로 중복 적용 제거
  clickIncome = Math.floor(baseClick * getGradeClickMultiplier() * trait.click * newbieBoost * (1 + getOptionBonus("click") / 100) * (1 + getOptionBonus("gold") / 100));
...
function getOptionLimitByGrade(grade = currentWand().grade) {
  return ({ E: 0, D: 1, C: 1, B: 2, A: 2, S: 3, SS: 4, SSS: 5 })[grade] ?? 0;
}
function markOptionDex(list = options) {
  if (!optionDex || typeof optionDex !== "object") optionDex = {};
  list.forEach(o => { if (o && o.key) optionDex[o.key] = true; });
}
function renderOptionDex() {
  const box = $("optionDexList");
  if (!box) return;
  if (!optionDex || typeof optionDex !== "object") optionDex = {};
  box.innerHTML = optionTypes.map(o => '<div class="option-dex-item ' + (optionDex[o.key] ? 'owned' : '') + '"><b>' + (optionDex[o.key] ? '✅' : '⬜') + ' ' + o.name + '</b><span>' + o.desc + '</span></div>').join("");
  const count = optionTypes.filter(o => optionDex[o.key]).length;
  setText("optionDexCount", count + " / " + optionTypes.length);
}

function tryAddRandomOption() {
  const maxOptions = getOptionLimitByGrade(currentWand().grade);
  if (maxOptions <= 0 || options.length >= maxOptions) return false;
  if (Math.random() > 0.35) return false;

  const type = pickWeightedOptionType();
  const value = Math.floor(type.min + Math.random() * (type.max - type.min + 1));
  options.push({ key: type.key, name: type.name, value });
  markOptionDex([{ key: type.key }]);

  $("message").innerText = "🎲 랜덤 옵션 획득! " + type.name + " +" + value + "%";
  return true;
}

function gradeSellPrice(grade, item = null) {
  const base = ({ E: 220, D: 650, C: 1500, B: 3600, A: 8500, S: 21000, SS: 56000, SSS: 150000 })[grade] || 200;
  const lv = item ? Math.max(0, item.savedLevel || 0) : 0;
  return Math.floor(base * (1 + lv * 0.06));
}

function gradeGemSellPrice(grade) {
  return ({ E: 0, D: 1, C: 2, B: 4, A: 8, S: 15, SS: 30, SSS: 60 })[grade] ?? 0;
}

function gradeStoneSellPrice(grade, item = null) {
  const base = ({ E: 0, D: 1, C: 2, B: 3, A: 5, S: 8, SS: 12, SSS: 20 })[grade] || 0;
  const lv = item ? Math.floor(Math.max(0, item.savedLevel || 0) / 5) : 0;
  return base + lv;
}
...
function addInventoryItem(wand, customName, protectedItem = false, savedLevel = null, savedOptions = null) {
  ...
  inventory.push({
    ...
    stoneSellPrice: gradeStoneSellPrice(wand.grade),
...
function sellItem(id) {
  ...
  const stoneReward = item.stoneSellPrice ?? gradeStoneSellPrice(item.grade, item);
  if (!confirm("[" + item.grade + "] " + item.name + "을(를) 판매할까요?\n골드 +" + item.sellPrice.toLocaleString() + " / 보석 +" + gemReward + " / 마력석 +" + stoneReward)) return;
  gold += item.sellPrice;
  gem += gemReward;
  stone += stoneReward;
  inventory = inventory.filter(x => x.id !== id);
  $("message").innerText = "🪙 " + item.name + " 판매 완료! 골드 +" + item.sellPrice.toLocaleString() + " · 보석 +" + gemReward + " · 마력석 +" + stoneReward;
  update();
}
...
function sellItems(mode) {
  ...
  const totalStone = targets.reduce((sum, x) => sum + (x.stoneSellPrice ?? gradeStoneSellPrice(x.grade, x)), 0);
  gold += total;
  gem += totalGem;
  stone += totalStone;
  inventory = inventory.filter(x => !ids.has(x.id));
  $("message").innerText = "🪙 마법봉 " + targets.length + "개 판매! 골드 +" + total.toLocaleString() + " · 보석 +" + totalGem + " · 마력석 +" + totalStone;
  update();
}
...
function summonOnce() {
  ...
  const itemId = addInventoryItem(picked, isNew ? picked.name : randomName, false);
  const newItem = inventory.find(x => x.id === itemId);
  if (newItem) {
    const optionCount = getOptionLimitByGrade(newItem.grade);
    newItem.savedOptions = [];
    for (let i = 0; i < optionCount; i++) {
      const type = pickWeightedOptionType();
      const value = Math.floor(type.min + Math.random() * (type.max - type.min + 1));
      newItem.savedOptions.push({ key: type.key, name: type.name, value });
    }
    markOptionDex(newItem.savedOptions);
  }
  return {
    ...picked,
    isNew,
...
function getRenderSignature() {
  return [
    ...
    unlocked.join(","), Object.keys(optionDex || {}).filter(k => optionDex[k]).sort().join(",")
  ].join(";");
}
...
function update(forceFullRender = false) {
  ...
  setText("successRate", getSuccessRate().toFixed(1));
  setText("mainClickIncome", clickIncome.toLocaleString());
  setText("mainAutoIncome", autoIncome.toLocaleString());
  setText("dpsIncome", autoIncome.toLocaleString());
  setText("mainDpsIncome", autoIncome.toLocaleString());
  setText("critChance", getCritChance().toFixed(1).replace(/\.0$/, "") + "%");
  setText("mainCritChance", getCritChance().toFixed(1).replace(/\.0$/, "") + "% / x" + getCritMultiplier().toFixed(2).replace(/\.00$/, ""));
  setText("comboBonus", "x" + getComboMultiplier().toFixed(2).replace(/\.00$/, ""));
  setText("powerScore", getPowerScore().toLocaleString());
  setText("mainSuccessRate", getSuccessRate().toFixed(1));
  setText("enhanceCost", enhanceCost.toLocaleString());
  setText("enhanceCostMini", enhanceCost.toLocaleString() + "골드 · " + getSuccessRate().toFixed(1) + "%");
...
function mineGold() {
  ...
  const now = Date.now();
  clickCombo = now - lastClickTime < 900 ? Math.min(24, clickCombo + 1) : 1;
  lastClickTime = now;
  totalClicks += 1;

  let earned = Math.max(1, Math.floor(clickIncome * getComboMultiplier()));
  if (level < 15 && gold < enhanceCost) earned = Math.floor(earned * 1.25);

  const isCrit = Math.random() * 100 < getCritChance();
  if (isCrit) {
    earned = Math.floor(earned * getCritMultiplier());
    totalCrits += 1;
    bestCrit = Math.max(bestCrit, earned);
  }
  ...
  const critText = isCrit ? "💥 CRITICAL x" + getCritMultiplier().toFixed(2).replace(/\.00$/, "") + "! " : "";
  const comboText = clickCombo >= 2 ? " · 콤보 x" + getComboMultiplier().toFixed(2).replace(/\.00$/, "") : "";
  $("message").innerText = shownGem > 0
    ? critText + "💎 보석 획득! 골드 +" + earned.toLocaleString() + " · 보석 +" + shownGem + comboText
    : critText + "💰 마법봉 채굴! 골드 +" + earned.toLocaleString() + comboText;
  syncEquippedFromCurrent();
  playMineFx(earned, shownGem);
  if (isCrit) showDopamineText("CRITICAL +" + earned.toLocaleString(), getRarityTier(wand.grade) >= 2 ? "rare" : "success");
  animateWand("success");
  const area = $("mineArea");
  if (area) {
    area.classList.add("mining");
    if (isCrit) area.classList.add("crit-hit");
    setTimeout(() => area.classList.remove("mining", "crit-hit"), 220);
  }
  update();
  save();
}
...
$("enhanceBtn").onclick = () => {
  ...
  let gain = 0;
  let resultLabel = "";
  if (roll < Math.min(2, rate * 0.06)) { success = true; gain = 3; resultLabel = "🌈 초월 성공!!!"; }
  else if (roll < Math.min(8, rate * 0.18)) { success = true; gain = 2; resultLabel = "✨ 대성공!!"; }
  else if (roll < rate) { success = true; gain = 1; resultLabel = "⚡ 강화 성공!"; }

  if (success) {
    level += gain;
    stone += Math.floor(1 + level / 5);
    const bonusGem = level % 5 === 0 ? 20 + Math.floor(level / 10) * 5 : 0;
    if (bonusGem) gem += bonusGem;
    enhanceCost = clampEnhanceCost(enhanceCost * (gain >= 3 ? 1.22 : gain === 2 ? 1.17 : 1.135) + 45);
    const gotOption = tryAddRandomOption();
    $("message").innerText = resultLabel + " +" + level + " 달성" + (gain > 1 ? " · 강화 +" + gain : "") + (bonusGem ? " · 보석 +" + bonusGem : "") + (gotOption ? " · 옵션 획득" : "");
    animateWand(gain >= 2 ? "rare" : "success");
  } else {
    const penalty = getFailPenalty();
    const failStone = Math.max(1, Math.floor(Math.max(1, level) / 5));
    level = Math.max(0, level - penalty);
    stone += failStone;
    enhanceCost = clampEnhanceCost(enhanceCost * 1.025 + 18);
    $("message").innerText = (penalty === 0 ? "💥 강화 실패... 단계 유지" : "💥 강화 실패... 단계 -" + penalty) + " · 실패 보상 마력석 +" + failStone;
    animateWand("fail");
  }
...
if ($("trainBtn")) $("trainBtn").onclick = () => { $("message").innerText = "0.69.1에서 성장 훈련은 제거되었습니다."; };
if ($("feedBtn")) $("feedBtn").onclick = () => { $("message").innerText = "0.69.1에서 성장 훈련은 제거되었습니다."; };
...
function getStateSnapshot() {
  syncEquippedFromCurrent();
  return { version: SAVE_VERSION, gold, gem, stone, level, enhanceCost, autoCost, unlocked, options, optionDex, inventory, itemSeq, equippedItemId, autoGoldGauge, lifetimeGoldGauge, totalClicks, totalCrits, bestCrit };
}
...
function applyStateSnapshot(g = {}) {
  ...
  options = Array.isArray(g.options) ? g.options.filter(o => o && optionTypes.some(t => t.key === o.key)).slice(0, 5) : [];
  optionDex = (g.optionDex && typeof g.optionDex === "object") ? g.optionDex : {};
  markOptionDex(options);
  ...
  totalClicks = toSafeNumber(g.totalClicks, 0);
  totalCrits = toSafeNumber(g.totalCrits, 0);
  bestCrit = toSafeNumber(g.bestCrit, 0);
  ensureStarterInventory();
}
...
function migrateSave(g = {}) {
  ...
  if (!migrated.optionDex || typeof migrated.optionDex !== "object") migrated.optionDex = {};
...
function load() {
  ...
  try {
    ...
    unlocked = [0]; options = []; optionDex = {}; inventory = []; itemSeq = 1; equippedItemId = null; totalClicks = 0; totalCrits = 0; bestCrit = 0; clickCombo = 0;
...
// 0.69.3: 모바일 채굴 터치 안정화
function bindMiningEventsStable(){
  const targets=[$("mineTapTarget"),$("mineArea"),$("wand"),$("wandImg"),$("workBtn")].filter(Boolean);
  targets.forEach(target=>{
    if(target.dataset.mineStableBound==="1") return;
    target.dataset.mineStableBound="1";
    ["pointerup","touchend","click"].forEach(evt=>{
...
// 장식 레이어가 터치를 먹어도 최종적으로 채굴 처리
document.addEventListener("click",(e)=>{
  const t=e.target;
  if(t&&t.closest&&t.closest("#mineArea")){
    handleMineInput(e);
  }
},true);
...
bindThemeEvent();
bindUiEvents();
bindMiningEventsStable();
cacheDom();
load();
update();

// 0.69.5: 모바일 우선 터치 채굴 보강
(function(){
  function bind0695(){
    if (typeof $ !== 'function' || typeof handleMineInput !== 'function') return;
    const targets = [$("mineTapTarget"), $("mineArea"), $("wand"), $("wandImg"), $("workBtn")].filter(Boolean);
    const events = ["pointerdown", "pointerup", "touchstart", "touchend", "click"];
    targets.forEach(target => {
      if (target.dataset.mineBound0695 === "1") return;
      target.dataset.mineBound0695 = "1";
      events.forEach(evt => target.addEventListener(evt, e => {
        if (e && typeof e.preventDefault === "function") e.preventDefault();
        if (e && typeof e.stopPropagation === "function") e.stopPropagation();
        handleMineInput(e);
      }, { passive:false, capture:true }));
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bind0695);
  else bind0695();
})();
