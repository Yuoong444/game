...
function playSummonReveal(result, count, newCount, stoneReward) {
  const stage = $("summonStage");
  const icon = $("summonResultIcon");
  const img = $("summonResultImg");
  if (!stage || !img || !icon) return;
...
function summon(count) {
  ...
  playSummonReveal(last, count, newCount, stoneReward);
...
function save() {
  localStorage.setItem("magicWandFixedGamePlus", JSON.stringify({
    gold, gem, stone, level, enhanceCost, autoCost, unlocked, options, growthLevel, growthExp, inventory, itemSeq, equippedItemId
  }));
}
