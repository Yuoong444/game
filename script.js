...
function update() {
  ...
  if ($("mainWandName")) $("mainWandName").innerText = wand.name;
  if ($("mainStars")) $("mainStars").innerText = renderStars(wand.stars);
  if ($("mainWeaponImg")) { $("mainWeaponImg").src = gradeImagePath(wand.grade); $("mainWeaponImg").alt = wand.name + " 이미지"; }
  ...
  if ($("mainClickIncome")) $("mainClickIncome").innerText = clickIncome.toLocaleString();
  if ($("mainAutoIncome")) $("mainAutoIncome").innerText = autoIncome.toLocaleString();
  if ($("mainSuccessRate")) $("mainSuccessRate").innerText = getSuccessRate();
  $("enhanceCost").innerText = enhanceCost.toLocaleString();
  if ($("enhanceCostMini")) $("enhanceCostMini").innerText = enhanceCost.toLocaleString() + " 골드 · 성공 " + getSuccessRate() + "%";
  $("autoCost").innerText = autoCost.toLocaleString();
  if ($("gradeBonusText")) $("gradeBonusText").innerText = "등급 x" + getGradeClickMultiplier().toFixed(2).replace(/\.00$/, "");
  if ($("gradeBonusInfo")) $("gradeBonusInfo").innerText = "x" + getGradeClickMultiplier().toFixed(2).replace(/\.00$/, "");
  if ($("gemDropRate")) $("gemDropRate").innerText = getGemDropRatePercent().toFixed(1).replace(/\.0$/, "") + "%";
  if ($("autoGemDropRate")) $("autoGemDropRate").innerText = getAutoGemDropRatePercent().toFixed(1).replace(/\.0$/, "") + "%";
  if ($("autoGemPity")) $("autoGemPity").innerText = autoIncome > 0 ? Math.max(0, getAutoGemPityNeed() - Math.floor(autoGoldGauge)).toLocaleString() : "-";
  if ($("clickBonusBadge")) $("clickBonusBadge").innerText = "x" + getGradeClickMultiplier().toFixed(1).replace(/\.0$/, "");
...
function getGemDropRatePercent() {
  return Math.min(16, 2.5 + (getGradeClickMultiplier() - 1) * 1.8);
}

function getAutoGemDropRatePercent() {
  const stoneBonus = Math.min(3.5, stone * 0.035);
  return Math.min(10, 1.2 + (getGradeClickMultiplier() - 1) * 1.05 + stoneBonus);
}
...
function mineGold() {
  ...
  const wand = currentWand();
  const gemChance = Math.min(0.16, 0.025 + (getGradeClickMultiplier() - 1) * 0.018);
  let bonusGem = 0;
  gold += earned;
  lifetimeGoldGauge += earned;
  if (lifetimeGoldGauge >= 30000) {
    const pityGem = Math.floor(lifetimeGoldGauge / 30000);
  ...
  if (Math.random() < gemChance) {
    bonusGem = wand.grade === "SSS" ? 3 : (wand.grade === "SS" || wand.grade === "S" ? 2 : 1);
    gem += bonusGem;
  }
  $("message").innerText = bonusGem > 0
    ? "💎 보석 광맥 발견! 골드 +" + earned.toLocaleString() + " · 보석 +" + bonusGem + " · 성장 EXP +1"
    : "💰 마법봉 채굴! 골드 +" + earned.toLocaleString() + " · 성장 EXP +1";
  playMineFx(earned, bonusGem);
  animateWand("success");
  const area = $("mineArea");
  if (area) {
...
setInterval(() => {
  if (autoIncome > 0) {
    ...
    if (autoReward.gem > 0) {
      $("message").innerText = autoReward.pity
...
function save() {
  localStorage.setItem("magicWandFixedGamePlus", JSON.stringify({
    gold, gem, stone, level, enhanceCost, autoCost, unlocked, options, growthLevel, growthExp, inventory, itemSeq, equippedItemId, autoGoldGauge, lifetimeGoldGauge
  }));
}
...
function load() {
  ...
  try {
    ...
    autoGoldGauge = g.autoGoldGauge ?? 0;
    lifetimeGoldGauge = g.lifetimeGoldGauge ?? 0;
    ensureStarterInventory();
  } catch (e) {}
}
