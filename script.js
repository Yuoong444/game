...
const gradeClickMultiplier = { E: 1.0, D: 1.20, C: 1.50, B: 1.95, A: 2.60, S: 3.60, SS: 5.10, SSS: 7.50 };
const gradeGoldBaseBonus = { E: 0, D: 35, C: 90, B: 180, A: 340, S: 650, SS: 1150, SSS: 2200 };
const gradeGoldLevelBonus = { E: 12, D: 16, C: 22, B: 30, A: 42, S: 60, SS: 88, SSS: 135 };
const gradeStartEnhanceCost = { E: 100, D: 180, C: 320, B: 580, A: 980, S: 1700, SS: 3200, SSS: 6200 };
...
const weaponTraits = {
  ...
  SSS: { name: "신화형", click: 1.55, auto: 1.45, gem: 1.55, success: 5 }
};
...
const SAVE_KEY = "magicWandRpgSaveV54";
const LEGACY_SAVE_KEYS = ["magicWandRpgSaveV53", "magicWandRpgSaveV52", "magicWandRpgSaveV51", "magicWandRpgSaveV50", "magicWandRpgSaveV49", "magicWandRpgSaveV48", "magicWandRpgSaveV47", "magicWandRpgSaveV46", "magicWandRpgSaveV45", "magicWandRpgSaveV44", "magicWandRpgSaveV35", "magicWandFixedGamePlus", "magicWandFixedGame"];
...
function calculateIncome() {
  ...
  const baseClick = 10 + gradeBase + level * gradeLevelBonus;
  clickIncome = Math.floor(baseClick * getGradeClickMultiplier() * trait.click * (1 + getOptionBonus("click") / 100));

  // v46: 자동 수익도 장착 무기의 등급 특성/강화 수치가 체감되도록 조정
  const stonePower = Math.floor(Math.sqrt(Math.max(0, stone)) * (8 + getGradeClickMultiplier()));
  const weaponPower = Math.floor((level + 1) * (5 + getGradeClickMultiplier() * 1.2));
  autoIncome = Math.floor((weaponPower + stonePower) * trait.auto * (1 + getOptionBonus("auto") / 100));
}
...
function renderInventory() {
  ...
  viewItems.forEach(item => {
    ...
    row.innerHTML = '<div class="inventory-icon"><img src="' + gradeImagePath(item.grade) + '" alt="" /></div>' +
      ...
      '<span>개별 강화 +' + (item.savedLevel ?? 0) + ' · 특성 ' + (item.trait || getWeaponTrait(item.grade).name) + ' · 다음비용 ' + (item.enhanceCost ?? (gradeStartEnhanceCost[item.grade] || 100)).toLocaleString() + optionText + ' · ' + (item.protected ? '기본 지급 · 판매 불가' : '판매가 ' + item.sellPrice.toLocaleString() + ' 골드 · 보석 ' + (item.gemSellPrice ?? gradeGemSellPrice(item.grade))) + '</span></div>' +
...
function update(forceFullRender = false) {
  ...
  setText("autoGemPity", (30000 - Math.floor(lifetimeGoldGauge % 30000)).toLocaleString());
...
function getGemDropRatePercent() {
  return Math.min(24, (2.5 + (getGradeClickMultiplier() - 1) * 1.8 + getOptionBonus("gem") / 10) * getWeaponTrait().gem);
}

function getAutoGemDropRatePercent() {
  const stoneBonus = Math.min(3.5, stone * 0.035);
  return Math.min(14, (1.2 + (getGradeClickMultiplier() - 1) * 1.05 + stoneBonus + getOptionBonus("gem") / 20) * getWeaponTrait().gem);
}
...
function mineGold() {
  ...
  syncCurrentFromEquipped();
  calculateIncome();
  const earned = Math.max(1, Math.floor(clickIncome * (1 + getOptionBonus("gold") / 100)));
  const wand = currentWand();
  const gemChance = Math.min(0.24, (0.025 + (getGradeClickMultiplier() - 1) * 0.018 + getOptionBonus("gem") / 1000) * getWeaponTrait().gem);
  let randomGem = 0;
  gold += earned;
...
/* v54: unified theme handling */
const THEME_KEY = "magicWandRpgThemeV54";
function getSavedTheme() {
  let saved = "light";
  try { saved = localStorage.getItem(THEME_KEY) || localStorage.getItem("magicWandRpgThemeV53") || "light"; } catch (e) {}
