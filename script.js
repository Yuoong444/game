...
let todayQuestProgress = 0;
let todayQuestClaimed = false;
let lastQuestKey = "";
...
const v63Quests = [
  { key: "click", title: "마법봉 30회 채굴", need: 30, reward: { gold: 2500, gem: 8 }, desc: "터치 플레이 보상" },
  { key: "crit", title: "크리티컬 5회 터뜨리기", need: 5, reward: { gold: 5000, stone: 6 }, desc: "대박 클릭 보상" },
  { key: "enhance", title: "강화 3회 성공", need: 3, reward: { gem: 12, stone: 8 }, desc: "성장 보상" },
  { key: "auto", title: "자동 수익 20,000골드 누적", need: 20000, reward: { gem: 10, stone: 12 }, desc: "방치 성장 보상" }
];

function pickWeightedOptionType() {
  const total = optionTypes.reduce((sum, x) => sum + (x.weight || 1), 0);
...
function ensureQuest() {
  const key = getQuestKey();
  if (lastQuestKey !== key) {
    lastQuestKey = key;
    // 초반(마력석 자동수익 없는 경우)에는 "자동 수익" 퀘스트 제외
    // 연속 중복 방지: 이전 인덱스와 다른 것 선택
    const isEarlyGame = autoIncome <= 0;
    const available = v63Quests.map((_, i) => i).filter(i => {
      if (isEarlyGame && v63Quests[i].key === "auto") return false;
      if (i === todayQuestIndex) return false; // 연속 중복 방지
      return true;
    });
    const pool = available.length > 0 ? available : v63Quests.map((_, i) => i).filter(i => i !== todayQuestIndex);
    todayQuestIndex = pool[Math.floor(Math.random() * pool.length)] ?? 0;
    todayQuestProgress = 0;
    todayQuestClaimed = false;
  }
}

function activeQuest() {
  ensureQuest();
  return v63Quests[todayQuestIndex] || v63Quests[0];
}

function addQuestProgress(type, amount) {
  const q = activeQuest();
  if (todayQuestClaimed || q.key !== type) return;
  const wasComplete = todayQuestProgress >= q.need;
  todayQuestProgress = Math.min(q.need, todayQuestProgress + amount);
  // 목표 달성 순간 자동 보상 지급
  if (!wasComplete && todayQuestProgress >= q.need) {
    autoClaimQuestReward();
  }
}

function autoClaimQuestReward() {
  const q = activeQuest();
  if (todayQuestClaimed) return;
  gold += q.reward.gold || 0;
  gem += q.reward.gem || 0;
  stone += q.reward.stone || 0;
  todayQuestClaimed = true;
  $("message").innerText = "🎯 성장 목표 달성! 자동 보상 지급!" + formatReward(q.reward);
  addEffect(true);
  spawnDopamineBurst("success", 56);
  flashDopamine("rare");
}

function formatReward(reward) {
  const arr = [];
  if (reward.gold) arr.push("골드 +" + reward.gold.toLocaleString());
  if (reward.gem) arr.push("보석 +" + reward.gem);
  if (reward.stone) arr.push("마력석 +" + reward.stone);
...
function cacheDom() {
  [
    ...
    "autoGemPity", "clickBonusBadge", "failPenalty", "stars", "gradeBadge", "inventoryList", "inventoryCount", "dpsIncome", "mainDpsIncome", "critChance", "mainCritChance", "comboBonus", "questTitle", "questProgress", "questReward", "questBar", "powerScore"
  ].forEach(id => ui[id] = $(id));
}
...
function getSuccessRate() {
  ...
  const raw = Math.min(96, base + getOptionBonus("rate") + getWeaponTrait().success + getElementSuccessBonus());
  return Math.round(raw * 10) / 10; // 소수점 첫째 자리까지
}
...
function renderQuest() {
  const q = activeQuest();
  if (!q || !$("questTitle")) return;
  const progress = Math.min(q.need, Math.floor(todayQuestProgress));
  const pct = Math.min(100, Math.floor(progress / q.need * 100));
  setText("questTitle", todayQuestClaimed ? "✅ " + q.title : q.title);
  setText("questProgress", progress.toLocaleString() + " / " + q.need.toLocaleString());
  setText("questReward", todayQuestClaimed ? "🎁 보상 지급 완료" : q.desc + formatReward(q.reward));
  const bar = $("questBar");
  if (bar) bar.style.width = pct + "%";
  // 버튼 대신 상태 표시 전용 — 완료 시 숨김
  const btn = $("questClaimBtn");
  if (btn) {
    btn.style.display = todayQuestClaimed ? "none" : "none"; // 자동 지급이므로 항상 숨김
  }
  const card = $("questClaimBtn") && $("questClaimBtn").closest(".v63-quest-card");
  if (card) card.classList.toggle("quest-done", todayQuestClaimed);
}
...
function update(forceFullRender = false) {
  ...
  setText("enhanceCostMini", enhanceCost.toLocaleString() + " 골드 · 성공 " + getSuccessRate().toFixed(1) + "%");
...
function mineGold() {
  ...
  addGrowthExp(isCrit ? 3 : 2);
  addQuestProgress("click", 1);
...
// questClaimBtn: 퀘스트 자동 지급 방식으로 변경되어 버튼 바인딩 제거
...
function getStateSnapshot() {
  syncEquippedFromCurrent();
  return { version: SAVE_VERSION, gold, gem, stone, level, enhanceCost, autoCost, unlocked, options, growthLevel, growthExp, inventory, itemSeq, equippedItemId, autoGoldGauge, lifetimeGoldGauge, totalClicks, totalCrits, bestCrit, todayQuestIndex, todayQuestProgress, todayQuestClaimed, lastQuestKey };
}
...
function applyStateSnapshot(g = {}) {
  ...
  todayQuestIndex = Math.floor(toSafeNumber(g.todayQuestIndex, 0));
  todayQuestProgress = toSafeNumber(g.todayQuestProgress, 0);
  todayQuestClaimed = !!g.todayQuestClaimed;
  lastQuestKey = g.lastQuestKey || "";
  ensureStarterInventory();
}
...
function load() {
  ...
  try {
    ...
    unlocked = [0]; options = []; growthLevel = 1; growthExp = 0; inventory = []; itemSeq = 1; equippedItemId = null; totalClicks = 0; totalCrits = 0; bestCrit = 0; todayQuestIndex = 0; todayQuestProgress = 0; todayQuestClaimed = false; lastQuestKey = ""; clickCombo = 0;
