...
const ui = {};
function cacheDom() {
  ["gold", "gem", "stone", "message", "wandName", "level", "nextLevel", "clickIncome", "autoIncome", "successRate", "enhanceCost", "enhanceCostMini", "autoCost", "inventoryList"].forEach(id => ui[id] = $(id));
}
...
function mineGold() {
  ...
  if (Math.random() < gemChance) {
    ...
    bonusGem += Math.floor(getOptionBonus("gem") / 3);
    gem += bonusGem;
  }
...
function applyStateSnapshot(g = {}) {
  ...
  enhanceCost = Math.max(100, Math.floor(toSafeNumber(g.enhanceCost, 100)));
