...
let options = [];
// growthLevel/growthExp: 0.68에서 제거됨 (잔재 변수 정리)
let inventory = [];
let itemSeq = 1;
...
const optionTypes = [
  ...
  { key: "crit", name: "크리티컬 확률", min: 2, max: 10, weight: 12, desc: "대박 클릭" },
  { key: "critDmg", name: "크리티컬 피해", min: 15, max: 55, weight: 8, desc: "크리 배율" }
];
...
const SAVE_VERSION = 691; // 0.69.1: animateWand rare 누적 수정, stoneSellPrice 마이그레이션, 다크테마 옵션도감, 동시해금 메시지, 잔재코드 정리
...
function getCritChance() {
  const gradeBonus = Math.max(0, (getGradeClickMultiplier() - 1) * 1.7);
  const elementBonus = normalizeAssetElement(getItemElement(getEquippedItem())) === "fire" ? 2 : normalizeAssetElement(getItemElement(getEquippedItem())) === "dark" ? 3 : 0;
  return Math.min(45, 5 + gradeBonus + getOptionBonus("crit") + elementBonus);
}

function getCritMultiplier() {
  const gradeBonus = Math.max(0, (getGradeClickMultiplier() - 1) * 0.08);
  return 2 + gradeBonus + getOptionBonus("critDmg") / 100;
}
...
function checkCollection(force = false) {
  ...
  if (newlyUnlocked.length === 1) {
    $("message").innerText = "📖 도감 등록! " + newlyUnlocked[0].name + " 해금!";
  } else if (newlyUnlocked.length > 1) {
    const grade = newlyUnlocked[0].grade;
    $("message").innerText = "📖 도감 등록! " + grade + "급 무기 " + newlyUnlocked.length + "종 해금! (" + newlyUnlocked.map(w => w.name).join(", ") + ")";
  }
}
...
function animateWand(type) {
  const wand = $("wand");
  wand.classList.remove("success", "fail", "rare");
...
// trainBtn/feedBtn: 0.68에서 HTML 제거됨
...
function migrateSave(g = {}) {
  ...
  migrated.inventory = migrated.inventory.map(item => ({
    ...
    stoneSellPrice: item.stoneSellPrice ?? gradeStoneSellPrice(item.grade, item),
