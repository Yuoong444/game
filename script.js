...
function syncEquippedProgress() {
  const item = getEquippedItem();
  if (!item) return;
  item.savedLevel = level;
  item.savedOptions = Array.isArray(options) ? JSON.parse(JSON.stringify(options)) : [];
}
...
function getFailPenalty() {
  // v24: 강화 실패 시 수익이 떨어지지 않도록 단계 하락을 제거했습니다.
  // 사용자가 '강화했는데 클릭 골드가 줄어드는' 느낌을 받지 않도록 실패는 단계 유지로 처리합니다.
  return 0;
}
