...
function playSummonReveal(result, count, newCount, stoneReward) {
  ...
  if (["S", "SS", "SSS"].includes(result.grade)) stage.classList.add("grade-" + result.grade.toLowerCase());
...
function renderInventory() {
  ...
  inventory.slice().reverse().forEach(item => {
    ...
    row.innerHTML = '<div class="inventory-icon"><img src="' + gradeImagePath(item.grade) + '" alt="" /></div>' +
      ...
      '<span>저장 강화 +' + (item.savedLevel ?? 0) + optionText + ' · ' + (item.protected ? '기본 지급 · 판매 불가' : '판매가 ' + item.sellPrice.toLocaleString() + ' 골드') + '</span></div>' +
